...
class Result<T, E> {
     ...
     */
    mapOrElse<U, D extends (value: E) => U, F extends (value: T) => U>(
        defaultValue: D,
        f: F,
    ): U {
        if (this.isOk()) {
     ...
     * This function can be used for control flow based on result values.
     */
    orElse<U, O extends (x: E) => Result<T, U>>(
        op: O,
    ): Result<T, U extends E ? U : E> {
        if (this.isErr()) {
    ...
    transpose(): Transpose<T, E> {
        if (this.isOk()) {
            ...
            return Some(Ok(this.value)) as any;
        }
        ...
        return Some(Err(this.error)) as any;
    }
}

type Transpose<T, E> = Option<
    Result<T extends Option<infer X> ? X : T, E extends Option<infer X> ? X : E>
>;
...
function Err<T, E>(value: E): Result<T, E> {
    return new Result(null as T, value);
}

/**
 * Try to execute a function and return a Result
 */
function Try<T extends (...args: any[]) => any, E>(
    op: T,
): ReturnType<T> extends Result<any, any>
    ? ReturnType<T>
