...
describe("Option", () => {
    ...
    describe("expect", () => {
        ...
        it("Panics if the value is a `None` with a custom panic message provided by `msg`.", () => {
            ...
            expect(() => x.expect("fruits are healthy")).toThrow(
                "fruits are healthy",
            );
        });
    });
    ...
    describe("map", () => {
        it("Maps an `Option<T>` to `Option<U>` by applying a function to a contained value.", () => {
            ...
            const maybeSomeLen: Option<number> = maybeSomeString.map(
                (s) => s.length,
            );
    ...
    describe("orElse", () => {
        ...
        it("Returns the option if it contains a value, otherwise calls `f` and returns the result.", () => {
            expect(Some("barbarians").orElse(vikings)).toEqual(
                Some("barbarians"),
            );
    ...
    describe("zipWith", () => {
        it("Returns `Some(f(s, o))` if `self` is `Some(s)` and `other` is `Some(o)`.", () => {
            ...
            expect(x.zipWith(y, (a, b) => ({ x: a, y: b }))).toEqual(
                Some({ x: 17.5, y: 42.7 }),
            );
        });
...
describe("Some", () => {
    ...
    it("Throws an error if called with null or undefined, but not with None.", () => {
        // @ts-expect-error
