...
import { Err, Ok, Result, Try } from "./result.ts";
import { ErrorKind } from "./util.ts";
...
describe("Result", () => {
    ...
    describe("isErrAnd", () => {
        it("returns true if the result is Err and the value inside of it matches a predicate", () => {
            ...
            expect(x.isErrAnd((x) => x.name === ErrorKind.RangeError)).toBe(
                true,
            );
        });
    ...
    describe("map", () => {
        it("maps a Result<T, E> to Result<U, E> by applying a function to a contained Ok value", () => {
            const parsed = fromJson<{ number: number }>('{"number": 1}');
            const json = parsed.map((x) => x.number);
            expect(json).toEqual(Ok(1));

            const parsed2 = fromJson<{ number: number }>('{"number": 1');
            const json2 = parsed2.map((x) => x.number);
            expect(json2.isErr()).toBe(true);
        });
    });
    ...
    describe("inspect", () => {
        it("Calls the provided closure with a reference to the contained value (if `Ok`)", () => {
            ...
            expect(x2.inspect((x) => log(`original: ${x}`))).toEqual(
                Err("foo"),
            );
    ...
    describe("inspectErr", () => {
        it("Calls the provided closure with a reference to the contained error (if `Err`)", () => {
            ...
            expect(x.inspectErr((x) => log(`original: ${x}`))).toEqual(
                Err("foo"),
            );
            expect(log).toHaveBeenCalled();
    ...
    describe("expect", () => {
        it("Returns the contained `Ok` value, consuming the `self` value.", () => {
            ...
            expect(() => x2.expect("Testing expect")).toThrow(
    ...
    describe("unwrapOrDefault", () => {
        it("Throws an UnimplementedError because it's not supported.", () => {
            const x: Result<number, string> = Ok(2);
            expect(() => x.unwrapOrDefault()).toThrow(
    ...
    describe("expectErr", () => {
        it("Returns the contained `Err` value, consuming the `self` value.", () => {
            ...
            expect(() => x2.expectErr("Testing expect_err")).toThrow(
                "Testing expect_err: 10",
            );
        });
    });
    ...
    describe("intoOk", () => {
        it("Throws an UnimplementedError because it is not yet supported.", () => {
            const x: Result<number, string> = Ok(2);
            expect(() => x.intoOk()).toThrow(
                "Result.intoOk() is not implemented",
            );
        });
    });
    ...
    describe("intoErr", () => {
        it("Throws an UnimplementedError because it is not yet supported.", () => {
            const x: Result<number, string> = Err("emergency failure");
            expect(() => x.intoErr()).toThrow(
                "Result.intoErr() is not implemented",
            );
        });
    });
    ...
    describe("andThen", () => {
        it("Calls `op` if the result is `Ok`, otherwise returns the `Err` value of `self`.", () => {
            ...
            const e = Err<unknown, string>("overflowed");
            expect(Ok(1_000_000_000).andThen(sqThenToString)).toEqual(e);
            expect(Err("not a number").andThen(sqThenToString)).toEqual(
                Err("not a number"),
            );
        });
    });
    ...
    describe("unwrapOrElse", () => {
        ...
        it("Returns the contained `Ok` value or computes it from a closure.", () => {
            expect(Ok<number, string>(2).unwrapOrElse(count)).toEqual(2);
    ...
    describe("transpose", () => {
        ...
        it("`Ok(Some(_))` and `Err(_)` will be mapped to `Some(Ok(_))` and `Some(Err(_))`.", () => {
            const x: Result<Option<number>, Error> = Err(
                new Error("Some error message"),
            );
            const y: Option<Result<number, Error>> = Some(
...
describe("Try", () => {
    it("Returns a `Result` containing the evaluation of the given function.", () => {
        const a1 = Try(() => 2 + 2);
        expect(a1).toEqual(Ok(4));

        const x = 1_000_000_000;

        expect(Try(() => x.checkedMul(x))).toEqual(Err("overflowed"));

        const actual1: Result<any, any> = Try(() => {
            throw new Error("Some error message");
        });
        expect(actual1).toEqual(Err(new Error("Some error message")));

        const actual2: Result<any, any> = Try(() => {
            throw "Some error message";
        });
        expect(actual2).toEqual(Err("Some error message"));
...
function fromJson<T>(s: string): Result<T, string> {
    try {
